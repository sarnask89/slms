    ...
    public function setupPerformanceAlert($modelId, $metric, $threshold, $condition = 'below') {
        try {
            $sql = "INSERT INTO ml_performance_alerts 
                    (model_id, metric, threshold, condition, is_active, created_at) 
        ...
        try {
            ...
            foreach ($activeAlerts as $alert) {
                ...
                if ($alert['condition'] === 'below' && $currentValue < $alert['threshold']) {
                    $triggered = true;
                } elseif ($alert['condition'] === 'above' && $currentValue > $alert['threshold']) {
                    $triggered = true;
                }
                ...
                if ($triggered) {
                    $alerts[] = [
                        ...
                        'message' => "Performance alert: {$alert['metric']} is {$alert['condition']} threshold ({$alert['threshold']}). Current value: {$currentValue}"
                    ];
                }
            }
