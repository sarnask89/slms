<?php
/**
 * Fix Config Paths Script - CORRECTED VERSION
 * Updates all modules to use just 'config.php' since they're already in modules folder
 */

echo "🔧 Fixing config.php paths in all modules (CORRECTED)...\n";
...
foreach ($files as $file) {
    ...
    foreach ($patterns as $pattern) {
        if (strpos($content, $pattern) !== false) {
            $content = str_replace($pattern, "require_once 'config.php';", $content);
            $fixed = true;
        }
    }
